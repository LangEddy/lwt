...
export function useExamples(wordId?: string) {
  ...
  const fetchExamples = useCallback(() => {
    if (!wordId) return
    setLoading(true)
    api.get<Example[]>(`/api/words/${wordId}/examples`)
      .then(data => {
        setExamples(data)
        setError(null)
      })
  ...
  const createExample = useCallback(async (example: {
    sentence: string
    translation?: string
    note?: string
  }) => {
    if (!wordId) throw new Error('No word selected')
    const newExample = await api.post<Example>(`/api/words/${wordId}/examples`, example)
    setExamples(prev => [newExample, ...prev])
    return newExample
  }, [wordId])

  const updateExample = useCallback(async (id: string, updates: {
    sentence?: string
    translation?: string
    note?: string
  }) => {
